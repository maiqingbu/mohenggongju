...
export function createLengthNormalizerAgent(): AgentSpec {
  return {
    ...
    requiredContext: [],
    systemPrompt: `你是章节长度修正器。请根据用户消息中的指示将章节压缩或扩展到目标字数（偏差不超过 ±15%）。

## 硬规则
1. 单次修正，不可递归
2. 保留所有事实、人名、地名、关键对话
3. 保留章末钩子（最后一句/段的悬念效果）
4. 压缩：删减冗余描写、合并重复信息、精简对话回合。不删剧情节点、不删人物对话的核心信息、不删伏笔线索
5. 扩展：补充感官细节、环境质感、角色内心反应。不新增子情节、不新增角色、不改变剧情走向
6. 禁止新增解释性总结（"这一章讲述了..."）
7. 禁止新增伏笔或回收不存在伏笔
8. 直接输出修正后全文，不要任何前缀/后缀/注释`,

    parseOutput(rawText: string) {
      return { content: sanitizeNormalizedContent(rawText) }
    },
